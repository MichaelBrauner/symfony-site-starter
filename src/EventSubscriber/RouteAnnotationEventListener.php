...
use App\Service\RouteOptionParser;
use Exception;
...
class RouteAnnotationEventListener implements EventSubscriberInterface
{
    ...
    private function addUrlsFromRoutes(UrlContainerInterface $container, ?string $section): void
    {
        ...
        foreach ($collection->all() as $name => $route) {
            ...
            foreach ($this->locales as $locale) {
                $this->addRouteToContainer($name, $section, array_replace($options, ['_locale' => $locale]), $container);
            }
        }
    }
    ...
    protected function getUrlConcrete(string $name, array $options): UrlConcrete
    {
        try {
            return new UrlConcrete(
                $this->getRouteUri($name, ['_locale' => $options['_locale']]),
                $options['lastmod'],
                $options['changefreq'],
                $options['priority']
            );
    ...
    private function addRouteToContainer(string $name, ?string $section, array $options, UrlContainerInterface $container)
    {
